...
describe("Admin Services", function () {
  ...
  it("should test if admin password can be updated", async function () {
    ...
    await db.none("INSERT INTO public.admins (username, password) VALUES ('bjorn123', 'some_hash')");
    
    // Update the password using the new function
    await admin.updatePassword('bjorn123', newHash);
    const updatedAdmin = await admin.getAdminByUsername('bjorn123');
