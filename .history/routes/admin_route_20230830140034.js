export default function admin_route(admin_service, waiter_service) {
  ...
  async function removeWaiter(req, res) {
    ...
    try {
      await waiter_service.removeWaiterFromShift(waiter, day, shift);  // No need to pass the request object
  
      // Update session to remove the checked day for this waiter
      const index = req.session.checkedDays.indexOf(`${day}-${shift}`);
      if (index > -1) {
