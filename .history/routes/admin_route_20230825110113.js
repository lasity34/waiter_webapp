export default function admin_route(admin_service, waiter_service) {
    ...
    try {
      ...
      if (admin) {
        ...
        if (isPasswordCorrect) {
          ...
          console.log('Setting admin username in session:', req.session.adminUsername);
  ...
  async function show(req, res) {
    ...
    console.log('Admin username from session:', req.session.adminUsername);

    const notification = req.session.notification
    res.render("admin", { username, waiters, schedule, notification });
  }
