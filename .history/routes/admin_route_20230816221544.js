export default function admin_route(admin_service, waiter_service) {
  ...
  async function createUser(req, res) {
    try {
      const { username, password } = req.body;
      const adminUsername = req.session.adminUsername || req.body.adminUsername; // Retrieve admin's username from session or hidden field
      await admin_service.createUser(username, password);
