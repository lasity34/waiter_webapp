export default function admin_route(admin_service, waiter_service) {
  ...
  async function deleteUser(req, res) {
    try {
      ...
      // Set the notification in the session
      req.session.notification = notification;
  
      // Render a temporary page with a meta-refresh to redirect after a delay
      res.render('redirect', { redirectUrl: `/admin/${adminUsername}`, notification });
    } catch (error) {
      console.error(error);
