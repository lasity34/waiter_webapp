...
export default function waiterService(db) {
  ...
  async function saveSelectedDays(username, selectedDays) {
    console.log("Saving selected days for username:", username); // Debugging log
    console.log("Selected days:", selectedDays); // Debugging log
    ...
    for (const day in selectedDays) {
      for (const time_slot in selectedDays[day]) {
        if (selectedDays[day][time_slot] === 'on') {
          console.log(`Inserting record for ${username}, ${day}, ${time_slot}`); // Debugging log
